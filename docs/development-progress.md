...
- 首页改为按会话显示明确入口：访客登录、成员管理资料、管理员进入后台；移除像项目说明的状态面板。登录页保留当前任务所需文案，增加密码显示控制、网络错误反馈与错误焦点定位。
- 管理后台新增应用/成员/操作记录导航，应用列表先于按需展开的创建表单；资料页删去重复的权限说明。设计取舍记录在 [界面信息层级](./reference/interface-ux.md)。
- 错误状态复测发现无效回调地址会触发 500 且创建按钮卡在加载态；已改为 400 校验响应和表单内明确错误。隔离 PostgreSQL 16 的 8 个 migration 从空库部署成功，完整 `pnpm oidc:smoke:phase4` 通过授权码 + PKCE、state、nonce、consent 与 claims。
- 最终 `pnpm validate` 63 项通过；使用临时独立构建 secret 的 `pnpm build` 通过。真实浏览器使用 disposable 管理员检查首页、登录、后台、资料页的桌面/手机布局、登录、密码显示、表单展开与无效地址反馈；手机 390px 页面无横向溢出。改动待 PR 合并与部署。

## 2026-09-26 邀请管理与撤回

- 管理员邀请页现可查看有效邀请和最近 30 条历史记录，按需刷新；待处理邀请支持二次确认撤回，成功后用户名和邮箱立即可再次分配，旧链接失效。列表和撤回 API 均复核活跃管理员权限，撤回 API 检查同源请求。
- 撤回使用条件更新和事务审计，不增加 schema 或 migration。`pnpm validate`、本地隔离 PostgreSQL 的 8 个既有 migration、邀请管理专项 2 项及临时安全配置下的 `pnpm build` 通过。真实浏览器验证撤回状态反馈、键盘提交、错误提示和手机/桌面无横向溢出。
- 本地实现待 PR 合并与部署；生产邀请邮件未在本次验证。
...
- 已实现管理员修改登录用户名、格式与大小写重名校验、有效邀请预留检查、Serializable 事务、审计及资料事件。无用户 UUID、OIDC sub 或权限迁移。
- 本地 `pnpm validate` 61 项通过；真实 PostgreSQL 专项 5 项通过；带临时独立安全配置的 `pnpm build`、改名后的完整 `pnpm oidc:smoke` 通过。
- Chromium 桌面 1440×1000、手机 390×844 验证保存、重名错误、格式约束、取消、Tab/Enter、权限与来源拒绝；无页面横向溢出或运行时异常。窄屏表格保持容器内横向滚动。
- 本次保留在本地，尚未提交、部署或验证生产事件投递。现有邮箱修改等未提交工作已保留。
...
- 管理员邮件邀请可指定全局用户名和 2 小时、1 天、7 天或 30 天有效期，成功后显示页面顶部通知；可查看、撤回未接受邀请并立即释放邮箱和用户名，到期后也会自动释放，受邀用户首次设置显示名和密码。
...
- 已接续工作区已有模型、摘要、领域函数和插件草稿，补齐 `/profile` 的密码确认、新邮箱 OTP、取消与刷新恢复；验证成功后旧邮箱收到安全提醒。
- 原子消费和邮箱、credential、审计、outbox 更新已放入同一事务；补齐部分唯一索引、失败次数约束与新 migration。旧登录 challenge 和恢复凭据失效，现有会话保留。
- `pnpm validate`（57 项通过）、`pnpm test:email-change`（6 项通过）和 `pnpm build` 通过；完整 OIDC 授权码 + PKCE smoke 通过。
- 隔离 PostgreSQL 16：8 个 migration 从空库部署成功；7 个旧 migration 的已有库应用新增 migration 成功。
- Chromium 1280×900 与 390×844 的真实密码/邮箱 OTP、键盘、错误提示、刷新、确认、取消通过，无横向溢出或页面运行时错误。
- 邮箱功能尚未提交、推送或部署到生产；生产邮件供应商与接入应用实际 webhook 投递未在本次验收。
