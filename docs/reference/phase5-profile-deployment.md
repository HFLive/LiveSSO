...
- 已登录且状态为 `ACTIVE` 的用户通过 `/profile` 管理个人资料；页面集中展示头像、显示名、用户名、邮箱验证状态、账号类型和加入时间。显示名通过 `PATCH /api/profile`、头像通过 `POST /api/profile/avatar` 由本人修改；用户名仍由管理员维护；邮箱可通过当前密码和新邮箱验证码自助更改，见 [邮箱更改契约](./phase3-auth-flows.md#自助更改邮箱)。
